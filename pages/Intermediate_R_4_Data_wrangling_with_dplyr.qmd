...
## The dplyr package: getting started

Often when working with data in R, you may need to reformat or 'wrangle' your data to get it in the form you need for your analysis. For example, you might need to filter out certain observations, select only a subset of variables, reorder or transform columns, or create summary statistics.

`dplyr` is an R package that helps with these tasks by providing functions for common data wrangling tasks, such as filtering rows, selecting specific columns, sorting, modifying or removing columns, and summarising data.

This kind of data wrangling is also *possible* - and in some cases is simpler - in base R, but dplyr offers more capability with consistent syntax, making it possible to carry out a wide range of data wrangling tasks in a consistent way.
...
### Overview of functions available in dplyr

`dplyr` offers a range of useful functions, each designed to perform a different data wrangling task. These functions are often referred to as "verbs" because they "do things" to data.
...
### Using dplyr functions: general syntax and the pipe operator
...
```{r}
#| eval = FALSE
_________ <- ________ |> ________________(________________, _________________ .....)
new data      data        dplyr fn name    first argument    second argument   etc.
```
...
- Type the name of the dplyr function you want to use in the blank space labelled `dplyr fn name`
...
- Usually you want to save your data once you have manipulated it into the right format. In the `new data` space, enter the name you want to give to the new data frame that is created.
...
```{r}
#| eval = FALSE
________ |> ________________(________________, _________________ .....)
 data        dplyr fn name    first argument    second argument   etc.
```
:::
...
These two pieces of code are of similar complexity, and either is completely valid. However, the real benefit of using the pipe operator comes when combining multiple dplyr functions together: allows you to write a sequence of data wrangling steps from left to right, creating clean and readable code that avoids nested function calls or the need to save intermediate objects (more on this later).
...
|-----------------|----------------------------------------------------|
| `filter(...)`   | Filters rows (choose rows based on column values). |
| `slice(...)`    | Chooses rows based on location.                    |
| `distinct(...)` | Removes duplicate rows.                            |
:::
...
|----------------|----------------|-----------------|-----------------------|
| `==` | equal to | `year == 2007` | The `year` column is equal to `2007` |
| `!=` | not equal to | `year != 2007` | The `year` column is not equal to `2007` |
| `>` | greater than | `pop > 1000000` | `pop` (population) is greater than `1,000,000` |
| `<` | less than | `pop < 1000000` | `pop` (population) is less than `1,000,000` |
| `>=` | greater than or equal to | `lifeExp >= 70` | `lifeExp` (life expectancy) is greater than or equal to `70` |
| `<=` | less than or equal to | `lifeExp <= 70` | `lifeExp` (life expectancy) is less than or equal to `70` |
...
| `%in%` | value is in a list of values | `country %in% c("France", "Germany")` | The `country` column is either `"France"` or `"Germany"` |
:::
...
### Creating new columns from existing ones: mutuate()
...
### Producing summary statistics without dplyr
...
| `quantile(..., 0.25)` | Calculates the lower quartile. |
| `quantile(..., 0.75)` | Calculate the upper quartile. |
...
The argument to the function (`(...)`) should be the thing that you want to summarise. For example, we know that we can apply these functions to a column in a data frame by using the `$` operator to point to a particular column, e.g.
...
median(gapminder_2007$gdpPercap)    # median GDP (in 2007)
```
:::
...
Calculate 2007 median life expectancy:
...
However, one of the main benefits of using `summarise()` is that it makes it easy to calculate multiple summary statistics in a single piece of code. For example, if you also wanted the lower and upper quartiles of life expectancy, you could run:
...
gm_life_stats_2007 <- gapminder_2007 |> summarise(median_lifeExp = median(lifeExp),
                                                  lowerq_lifeExp = quantile(lifeExp, 0.25),
                                                  upperq_lifeExp = quantile(lifeExp, 0.75))
...
### Combining group_by() and summarise() to produce grouped summary statistics
...
___________ <- ________ |> group_by(________________) |>
 new data       data                 groups column  
                           summarise(_______________ = ____________(__________))
                                      summary name      summary fn   column
```
...
- In the `summary fn` space, enter the name of the summary function you want to use e.g. `mean`, `median` etc..
...
___________ <- ________ |> group_by(________________) |>
 new data       data                 groups column  
                           summarise(______________ = ____________(__________),
                                      summary name     summary fn   column
...
The following code produces the median, lower quartile and upper quartile of life expectancy, separately for each continent.
...
- `summarise` is the next dplyr function we apply to the data. Within this function:
  ...
  - `lowerq_lifeExp = quantile(lifeExp, 0.25)` creates a column called lowerq_lifeExp which contains the lower quartile of life expectancy
  - `upperq_lifeExp = quantile(lifeExp, 0.75)` creates a column called upperq_lifeExp which contains the upper quartile of life expectanc
...
## Combining multiple dplyr functions in a single command
...
Filter the `gapminder` data to produce a new data frame called `gapminder_post_1970` which only contains data from 1970 onwards.
...
### 2007 data with population in millions
...
- For some functions, there are simpler base R alternatives, but dplyr is often more readable, especially for multi-step data wrangling and grouped operations.
