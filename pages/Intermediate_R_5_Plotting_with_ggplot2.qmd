...
Note that the medians shown in the boxplot above are not the median life expectancies of the overall populations of each continent. Since the data contains one observation per country, the median represents the median across countries within each continent - countries with larger populations are not weighted any differently to smaller ones.

If presenting this plot formally, it would be worth including a footnote to make this clear.
:::

## Building a plot: components and customisation

Now let's explore the different components in more detail and how to use them to build the plot you want.

### Produce the base plot: data, axes and plot type

The first component - `ggplot()` together with the `aes()` function nested inside it - initialises (sets up) the plot. The first argument within `ggplot()` is the name of the data you want to use. The second argument is the `aes()` function (aes is short for aesthetics), which is used to specify which variables to plot on the x- and y-axis. The `aes()` function can also be used to map other visual properties to variables - this is covered in the next section.

![](../images/component1.png){width="600" style="display:block; margin: 2em auto;"}

The second component is the `geom_xxx()` function, which specifies the type of plot to create. "Geom" is short for geometry and refers to the geometric objects (e.g. points, bars, or lines) used to represent the data. `xxx` is replaced with the name of the geometry you want to use.

![](../images/component2.png){width="600" style="display:block; margin: 2em auto;"}


Some of the most commonly used geom functions are shown below:
...
Examples of this can be seen in the "Full examples" section and the Exercises.
:::
...
### Map variable aesthetics inside aes()
...
We have already used `x=` and `y=` inside the `aes()` function to map variables to the axes. The table below summarises this and some other commonly used aesthetics:
...
| `group=` | Specifies which observations belong to the same group, allowing ggplot to draw separate lines, polygons, or other grouped elements for each category |
...
| `shape=` | Assign different shapes to points in a scatter plot based on the specified variable |
...
```{r}
#| eval: false
ggplot(gapminder_2007, aes(x = continent, y = lifeExp, __________ = ___________)) +
...
Notice that R also adds a legend for us, to show which colour corresponds to which continent. We may not want to include this since the continents are already defined on the x-axis, and will see later how this can be removed.

## Scatter plot: colour

**Base plot**

The code below creates a scatter plot (`geom_point`) using the 2007 data (`gapminder_2007`), with population GDP per capita (`gdpPercap`) on the x axis and life expectancy (`lifeExp`) on the y axis:
...
Notice that R also adds a legend for us, to show which colour corresponds to which continent.

:::

::::

### Set fixed aesthetics inside geom_xxx()

So far we have used `geom_xxx()` with no arguments inside the brackets, but there are lots of additional customisation options available by specifying arguments inside it.

We saw in the previous section that aesthetics specified inside the `aes()`function map a *variable* to a visual property. Inside `geom_xxx()`, we can instead set *fixed* aesthetics, where a visual property is set to a constant value for that layer - for example, making all of the boxes filled in blue.
...
As seen previously, the code below creates a scatter plot (`geom_point`) using the 2007 data (`gapminder_2007`), withGDP per capita (`gdpPercap`) on the x axis and life expectancy (`lifeExp`) on the y axis. We saw that adding `colour = continent` inside the `aes()` function maps the continent variable to the colour aesthetic, meaning the points in the scatter plot are coloured according to their continent.
...
**Adding fixed aesthetics**

In the code below, we have added two fixed aesthetics inside the `geom_point` function:

- `size = 3` makes *all* of the points in the scatter plot size 3 (the default size is 1.5)
- `alpha = 0.6` makes *all* of the points a bit transparent - this makes it easier to see the points when they overlap
...
::: {.callout-note icon="false" collapse="true"}
### [{{< fa info-circle >}}]{style="color: #9e9e9e;"}  aes() can be used inside geom_xxx() functions too

Normally, mappings between variables and visual properties are defined within the `aes()` functions, with `ggplot()` (i.e. `ggplot(x = ..., y = ..., aes(...)`). These mappings are then inherited by all layers.

However, you can also use `aes()` inside a specific `geom_xxx()` layer. In this case, the mapping applies *only to that layer*, leaving other layers unchanged.

This is useful when different layers need different aesthetic mappings.
:::
...
![](../images/component3.png){width="600" style="display:block; margin: 2em auto;"}
...
ggplot(gapminder_2007, aes(x = gdpPercap, y = lifeExp, colour = continent)) +
  ...
  labs(title = "Life expectancy vs GDP",
       ...
       y = "Life expectancy (years)",
       colour = "Continent") # The default legend title is the variable name (continent)
                             # Here we rename it so it has a capital C
```
:::
::::
...
::: {.callout-note icon="false" collapse="true"}
### [{{< fa info-circle >}}]{style="color: #9e9e9e;"}   Where can I see examples of using these components?

Type `help()` with the name of the function in the brackets into the console (e.g. `help(scale_x_continuous)`) to open the corresponding R help page. The examples at the bottom of the help page are often particularly useful for understanding how the function works.
:::
...
- The `ggplot()` function asks R to create a new plot
  ...
  - The next argument is the `aes()` function
    - Inside the `aes()` function, `x = continent` maps continent to the x-axis and `y = lifeExp` maps life expectancy to the y-axis.
- Adding the component "`geom_boxplot()`" tells R to produce a box plot.
...
- `alpha = 0.6` makes the points slightly transparent, helping to reveal overlapping points
...
- `colour =` is applied inside each geom\_ component separately - "steelblue" for the points and "coral" for the trend line
- `theme_minimal()` applies a clean theme with a minimal background
...
Using `dplyr` functions, create a data frame which:

- Contains only UK data (from all years)
- Has a new column giving population in millions 

Using the data frame you created above, produce a plot of UK population (in millions) over time:
...
ggplot(gapminder_uk_mil, aes(x = year, y = pop_mil)) +
  geom_line() +
  geom_point() +
...
### Life expectancy by continent in 1997 vs 2007
...
Suppose we want to compare life expectancy in 1997 vs 2007, separately for each continent.

- Create a data frame containing only 1997 and 2007 data, and convert `year` to a factor variable
- Create box plot with year on the x-axis and life expectancy on the y-axis
- Add a `facet_wrap()` component so that each continent is shown in a separate panel
- Add suitable labels
...
ggplot(gapminder_97_07, aes(x = year, y = lifeExp)) +
  geom_boxplot() +
  facet_wrap(~continent) +
  labs(title = "Life expectancy by continent in 1997 and 2007",
       x = "Year",
       y = "Life expectancy")
```
:::
